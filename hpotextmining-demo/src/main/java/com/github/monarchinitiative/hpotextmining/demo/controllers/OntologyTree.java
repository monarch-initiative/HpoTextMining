...
import com.github.monarchinitiative.hpotextmining.demo.util.WidthAwareTextFields;
...
public final class OntologyTree {
    ...
    /**
     * Expand and scroll to the term selected in the search text field.
     */
    @FXML
    public void goButtonAction() {
        String id = labels.get(searchTextField.getText());
        if (id != null) {
            expandUntilTerm(ontology.get().getTerm(id));
    ...
    public void initialize() {
        ...
        goButton.disableProperty().bind(ontology.isNull());
        searchTextField.disableProperty().bind(ontology.isNull());
        ontologyTreeView.disableProperty().bind(ontology.isNull());
        notPresentCheckBox.disableProperty().bind(ontology.isNull());
        addButton.disableProperty().bind(ontology.isNull());
        ontology.addListener(((observable, oldValue, newValue) -> {
            if (newValue == null) {
                labels.clear();
    ...
    void focusOnTerm(String termId) {
        Term term = ontology.get().getTerm(termId);
        if (term == null) {
    ...
    private void expandUntilTerm(Term term) {
        if (ontology.get().existsPath(ontology.get().getRootTerm().getID(), term.getID())) {
            // find root -> term path through the tree
            Stack<Term> termStack = new Stack<>();
            termStack.add(term);
            Set<Term> parents = ontology.get().getTermParents(term);
            while (parents.size() != 0) {
                Term parent = parents.iterator().next();
                termStack.add(parent);
                parents = ontology.get().getTermParents(parent);
            }
            ...
            LOGGER.warn(String.format("Unable to find the path from %s to %s", ontology.get().getRootTerm(),
                    term.getName()));
        }
    }
    ...
    class TermTreeItem extends TreeItem<Term> {
        ...
        public boolean isLeaf() {
            Set<Term> children = ontology.get().getTermChildren(getValue());
