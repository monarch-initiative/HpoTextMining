...
public final class Present {
    ...
    @FXML
    private ProgressIndicator miningProgressIndicator;

    @FXML
    private Label miningProgressLabel;
    ...
    public void initialize() {
        ...
        miningService.setExecutor((executor != null) ? executor : Executors.newSingleThreadExecutor());
    ...
    private void displayResults(MiningService service) {
        ...
        yesTerms = terms.stream()
                ...
                .map(term -> ontology.get().getTerm(term.getTermId()))
                .distinct()
    ...
    private String colorizeHTML(Set<MinedTerm> resultSet, String minedText) {
        ...
        for (MinedTerm result : sortedResults) {
            ...
            Term term = ontology.get().getTerm(result.getTermId());
            if (term == null)
                continue;
